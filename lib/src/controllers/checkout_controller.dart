...
class CheckoutController extends CartController {
  ...
  bool loading = true;
  int loadingUrl = 0;
  ValueNotifier<String> urlPay = ValueNotifier<String>("");
  ...
  void addOrder(List<Cart> carts) async {
    ...
    _order.hint = CartController.hint;
    _order.timeOrder = CartController.timeOrder;
    carts.forEach((_cart) {
  ...
  void getUrl(List<Cart> carts) async {
    setState(() {
      loading = true;
    });
    Order _order = new Order();
    ...
    orderRepo.getUrl(_order, this.payment!).then((value) async {
      settingRepo.coupon = new Coupon.fromJSON({});
      if (value != null) {
        setState(() {
          loading = false;
